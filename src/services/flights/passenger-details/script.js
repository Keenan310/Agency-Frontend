(function () {
  ...
  const REQUIRED_FIELD_KEYS = ['title', 'fname', 'lname', 'dob', 'nationality', 'gender', 'passport', 'passportExpiry'];
  ...
  let selectedOffer = {
    ...
    baggage: {
      cabin: { allowance: '7 KG', description: '1 cabin bag included with the selected fare option.' },
      checkIn: { allowance: '30 KG', description: 'Included check-in baggage allowance for this selected offer.' }
    },
    price: {
      currency: 'AED',
      baseFare: 1850,
  ...
  function loadBookingData() {
    ...
    try {
      ...
      selectedOffer.segments = flight.segments.map(seg => ({
        ...
        departureDate: new Date(seg.departureDateTime).toLocaleDateString('en-GB', { day: '2-digit', month: 'short', year: 'numeric' }),
        departureTime: new Date(seg.departureDateTime).toLocaleTimeString('en-GB', { hour: '2-digit', minute: '2-digit', hour12: false }),
        arrivalCode: seg.to,
        arrivalAirport: seg.toCity || seg.toAirport || seg.to,
        arrivalDate: new Date(seg.arrivalDateTime).toLocaleDateString('en-GB', { day: '2-digit', month: 'short', year: 'numeric' }),
        arrivalTime: new Date(seg.arrivalDateTime).toLocaleTimeString('en-GB', { hour: '2-digit', minute: '2-digit', hour12: false }),
        airline: seg.airline
      }));

      selectedOffer.baggage = {
        cabin: { allowance: flight.baggage?.cabin || '7 KG', description: 'Cabin baggage allowance' },
        checkIn: { allowance: fare.baggage || flight.baggage?.checked || '20 KG', description: 'Check-in baggage allowance' }
      };

      const total = fare.priceAmount || flight.priceAmount || 1850;
      ...
      selectedOffer.price.baseFare = total - (selectedOffer.price.taxesFees + selectedOffer.price.serviceFee);
  ...
  function initializeDefaultState() {
    if (!window.bookingState) {
      const total = selectedOffer.price.baseFare + selectedOffer.price.taxesFees + selectedOffer.price.serviceFee;
      window.bookingState = {
        offerId: selectedOffer.offerId,
        confirmedOfferId: null,
        haveBundles: false,
  ...
  function money(value) {
    return selectedOffer.price.currency + ' ' + Number(value || 0).toLocaleString('en-AE', {
  ...
  function createPassengersFromCounts() {
    ...
    for (let i = 1; i <= adults; i += 1) passengers.push({ type: 'ADT', label: 'Adult ' + i });
    for (let i = 1; i <= children; i += 1) passengers.push({ type: 'CHD', label: 'Child ' + i });
    for (let i = 1; i <= infants; i += 1) passengers.push({ type: 'INF', label: 'Infant ' + i });
  ...
  function passengerSummaryText() {
    const parts = [];
    if (passengerCounts.adults) parts.push(passengerCounts.adults + ' Adult' + (passengerCounts.adults > 1 ? 's' : ''));
    if (passengerCounts.children) parts.push(passengerCounts.children + ' Child' + (passengerCounts.children > 1 ? 'ren' : ''));
    if (passengerCounts.infants) parts.push(passengerCounts.infants + ' Infant' + (passengerCounts.infants > 1 ? 's' : ''));
  ...
  function getDefaultTitle(passengerType, gender) {
    return (TITLE_RULES[passengerType] && TITLE_RULES[passengerType][gender]) || '';
  }
  ...
  function bindPassengerFormEvents() {
    document.querySelectorAll('.passenger-form-card [data-field-key="gender"]').forEach(function (genderSelect) {
  ...
  function renderSelectedOfferDetails() {
    byId('flightAirlineName').textContent = (selectedOffer.segments[0] && selectedOffer.segments[0].airline) || 'Selected Flight';
    byId('selectedFareCategory').textContent = selectedOffer.fareCategory;

    byId('segmentList').innerHTML = selectedOffer.segments.map(function (segment) {
      return '' +
        '<div class="segment-item">' +
        '<div class="segment-route">' +
        '<div class="airport-block">' +
        '<div class="airport-label">Departure</div>' +
        '<div class="airport-code">' + escapeHtml(segment.departureCode) + '</div>' +
        '<div class="airport-name">' + escapeHtml(segment.departureAirport) + '</div>' +
        '<div class="airport-datetime">' +
        '<div class="airport-date">' + escapeHtml(segment.departureDate) + '</div>' +
        '<div class="airport-time">' + escapeHtml(segment.departureTime) + '</div>' +
        '</div>' +
        '</div>' +
        '<div class="route-arrow">→</div>' +
        '<div class="airport-block">' +
        '<div class="airport-label">Arrival</div>' +
        '<div class="airport-code">' + escapeHtml(segment.arrivalCode) + '</div>' +
        '<div class="airport-name">' + escapeHtml(segment.arrivalAirport) + '</div>' +
        '<div class="airport-datetime">' +
        '<div class="airport-date">' + escapeHtml(segment.arrivalDate) + '</div>' +
        '<div class="airport-time">' + escapeHtml(segment.arrivalTime) + '</div>' +
        '</div>' +
        '</div>' +
        '</div>' +
        '<div class="segment-id">Segment: ' + escapeHtml(segment.id) + ' · Fare: ' + escapeHtml(selectedOffer.fareCategory) + '</div>' +
        '<div class="segment-baggage">' +
        '<span class="bag-chip">Cabin: ' + escapeHtml(selectedOffer.baggage.cabin.allowance) + '</span>' +
        '<span class="bag-chip">Check-in: ' + escapeHtml(selectedOffer.baggage.checkIn.allowance) + '</span>' +
        '</div>' +
        '</div>';
    }).join('');

    const total = selectedOffer.price.baseFare
    ...
    byId('customerDiscount').textContent = '- ' + money(selectedOffer.price.customerDiscount);
    byId('totalPayable').textContent = money(total);
  }

  function renderPassengerForms() {
    const passengers = createPassengersFromCounts();
    byId('passengerCountTag').textContent = passengers.length + ' Passenger' + (passengers.length !== 1 ? 's' : '') + ' · ' + passengerSummaryText();
    byId('passengerPriceLabel').textContent = passengerSummaryText();

    const options = titleOptionsHtml();

    byId('passengerForms').innerHTML = passengers.map(function (passenger, index) {
      const isFirstPassenger = index === 0;
      const typeName = passengerTypeName(passenger.type);

      let html = '' +
        '<div class="passenger-form-card" data-passenger-index="' + index + '" data-passenger-type="' + escapeHtml(passenger.type) + '">' +
        '<div class="passenger-card-head">' +
        '<h3 class="passenger-card-title">' + escapeHtml(passenger.label) + ' Information</h3>' +
        '<span class="pd-pax-tag">Passenger ' + (index + 1) + ' · ' + escapeHtml(passenger.type) + '</span>' +
        '</div>' +
        '<section class="pd-section">' +
        '<div class="pd-section-title">Personal Information</div>' +
        '<div class="pd-grid g3">' +
        '<div class="pd-field">' +
        '<label for="' + fieldId(index, 'title') + '">Title</label>' +
        '<select id="' + fieldId(index, 'title') + '" data-field-key="title">' +
        '<option value="">Select title</option>' + options +
        '</select>' +
        '<div class="pd-error-msg"></div>' +
        '</div>' +
        '<div class="pd-field">' +
        '<label for="' + fieldId(index, 'fname') + '">First Name</label>' +
        '<input id="' + fieldId(index, 'fname') + '" data-field-key="fname" type="text" placeholder="' + escapeHtml(typeName) + ' first name" />' +
        '<div class="pd-error-msg"></div>' +
        '</div>' +
        '<div class="pd-field">' +
        '<label for="' + fieldId(index, 'lname') + '">Last Name</label>' +
        '<input id="' + fieldId(index, 'lname') + '" data-field-key="lname" type="text" placeholder="' + escapeHtml(typeName) + ' last name" />' +
        '<div class="pd-error-msg"></div>' +
        '</div>' +
        '<div class="pd-field">' +
        '<label for="' + fieldId(index, 'dob') + '">Date of Birth</label>' +
        '<input id="' + fieldId(index, 'dob') + '" data-field-key="dob" type="date" />' +
        '<div class="pd-error-msg"></div>' +
        '</div>' +
        '<div class="pd-field">' +
        '<label for="' + fieldId(index, 'nationality') + '">Nationality</label>' +
        '<input id="' + fieldId(index, 'nationality') + '" data-field-key="nationality" type="text" placeholder="Country code" />' +
        '<div class="pd-error-msg"></div>' +
        '</div>' +
        '<div class="pd-field">' +
        '<label for="' + fieldId(index, 'gender') + '">Gender</label>' +
        '<select id="' + fieldId(index, 'gender') + '" data-field-key="gender">' +
        ...
        '<div class="pd-grid g3">' +
        '<div class="pd-field">' +
        '<label for="' + fieldId(index, 'passport') + '">Passport Number</label>' +
        '<input id="' + fieldId(index, 'passport') + '" data-field-key="passport" type="text" placeholder="Passport number" />' +
        '<div class="pd-error-msg"></div>' +
        '</div>' +
        '<div class="pd-field">' +
        '<label for="' + fieldId(index, 'passportIssue') + '">Issue Date</label>' +
        '<input id="' + fieldId(index, 'passportIssue') + '" data-field-key="passportIssue" type="date" />' +
        '<div class="pd-error-msg"></div>' +
        '</div>' +
        '<div class="pd-field">' +
        '<label for="' + fieldId(index, 'passportExpiry') + '">Expiry Date</label>' +
        '<input id="' + fieldId(index, 'passportExpiry') + '" data-field-key="passportExpiry" type="date" />' +
        '<div class="pd-error-msg"></div>' +
        '</div>' +
        '</div>' +
        '</section>';
      ...
      if (isFirstPassenger) {
        html += '' +
          ...
          '<div class="pd-grid">' +
          '<div class="pd-field">' +
          '<label for="' + fieldId(index, 'email') + '">Email</label>' +
          '<input id="' + fieldId(index, 'email') + '" data-field-key="email" type="email" placeholder="passenger@email.com" />' +
          '<div class="pd-error-msg"></div>' +
          '</div>' +
          '<div class="pd-field">' +
          '<label for="' + fieldId(index, 'phone') + '">Phone</label>' +
          '<input id="' + fieldId(index, 'phone') + '" data-field-key="phone" type="tel" placeholder="971501234567" />' +
          '<div class="pd-error-msg"></div>' +
          '</div>' +
          '</div>' +
          '</section>';
  ...
  function validatePassengerForm() {
    ...
    passengerCards.forEach(function (card, index) {
      const requiredKeys = index === 0 ? REQUIRED_FIELD_KEYS.concat(CONTACT_FIELD_KEYS) : REQUIRED_FIELD_KEYS;
  ...
  function clearValidation() {
    document.querySelectorAll('.passenger-form-card input, .passenger-form-card select').forEach(function (el) {
  ...
  function getPassengerData() {
    return Array.from(document.querySelectorAll('.passenger-form-card')).map(function (card) {
      function value(key) {
        const el = card.querySelector('[data-field-key="' + key + '"]');
  ...
  function buildNdcPassengerPayload(offerId) {
    ...
    return {
      ...
      SegmentIds: selectedOffer.segments.map(function (segment) { return segment.id; }),
  ...
  function runSelfTests() {
    ...
    console.assert(document.querySelectorAll('.passenger-form-card').length === 3, 'Test 1 failed: 1 adult + 1 child + 1 infant should render 3 forms.');
    const payload = buildNdcPassengerPayload('TEST-OFFER-ID');
    console.assert(payload.OfferId === 'TEST-OFFER-ID', 'Test 2 failed: payload should use provided offer ID.');
    console.assert(payload.Passengers.length === 3, 'Test 3 failed: payload should include 3 passengers.');
    console.assert(payload.Passengers[0].PassengerTypeCode === 'ADT', 'Test 4 failed: first passenger should be ADT.');
    console.assert(payload.Passengers[1].PassengerTypeCode === 'CHD', 'Test 5 failed: second passenger should be CHD.');
    console.assert(payload.Passengers[2].PassengerTypeCode === 'INF', 'Test 6 failed: third passenger should be INF.');
    console.assert(selectedOffer.price.serviceFee === 50, 'Test 6A failed: service fee should exist in selectedOffer.price.');
    console.assert(selectedOffer.price.customerDiscount === 25, 'Test 6B failed: customer discount should exist in selectedOffer.price.');

    const firstCard = document.querySelector('.passenger-form-card[data-passenger-type="ADT"]');
    ...
    console.assert(firstTitle.value === 'MR', 'Test 7 failed: Adult Male should auto-select MR.');

    const childCard = document.querySelector('.passenger-form-card[data-passenger-type="CHD"]');
    ...
    console.assert(childTitle.value === 'MISS', 'Test 8 failed: Child Female should auto-select MISS.');

    const infantCard = document.querySelector('.passenger-form-card[data-passenger-type="INF"]');
    ...
    console.assert(infantTitle.value === 'MSTR', 'Test 9 failed: Infant Male should auto-select MSTR.');

    const femaleAdultTitle = getDefaultTitle('ADT', 'Female');
    console.assert(femaleAdultTitle === 'MRS', 'Test 10 failed: Adult Female should map to MRS.');
    setPassengerCounts({ adults: 2, children: 0, infants: 0 });
    console.assert(document.querySelectorAll('.passenger-form-card').length === 2, 'Test 11 failed: 2 adults should render 2 forms.');
    setPassengerCounts({ adults: 1, children: 1, infants: 1 });
    console.groupEnd();
  }
  ...
  byId('confirmSelectionBtn').addEventListener('click', async function() {
    ...
    try {
      ...
      const data = await apiPost('/flights/fare-confirm', { offerId: window.bookingState.offerId });
      console.log('Fare Confirmed:', data);

      window.bookingState.confirmedOfferId = data.offerId || data.confirmed?.offerId;
      window.bookingState.haveBundles = data.confirmed?.haveBundles || false;
      window.bookingState.cabin_held = data.confirmed?.canBeHeld || data.confirmed?.cabin_held || false;
      ...
      // Show next steps
      if (window.bookingState.haveBundles && data.bundles && data.bundles.length > 0) {
  ...
  byId('confirmBundlesBtn')?.addEventListener('click', function() {
    const selected = Array.from(document.querySelectorAll('input[name="bundleSelect"]:checked')).map(cb => ({
  ...
  function buildReviewFlightData() {
    ...
    const segments = (selectedOffer.segments || []).map(function (segment) {
      return {
        ...
        departureTime: toReviewDateTime(segment.departureDate, segment.departureTime),
    ...
    const bundleTotal = (window.bookingState.selectedBundles || []).reduce(function (sum, item) {
    ...
    const totalAmount = baseFare + taxes + ancillary + serviceFee - discount;
  ...
  function buildBookingReviewData(paxData) {
    ...
    return {
      ...
      offerId: window.bookingState.confirmedOfferId || window.bookingState.offerId || selectedOffer.offerId,
  ...
  byId('holdBtn')?.addEventListener('click', () => submitPassengersAndProceed('review'));
  byId('bookAndPayBtn')?.addEventListener('click', () => submitPassengersAndProceed('review'));
